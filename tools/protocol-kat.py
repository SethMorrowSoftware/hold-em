...
def main():
    ...
    if "--json" in sys.argv:
        # Pure JSON (no Python assignment prefix) so an implementation in ANY
        # language can consume these vectors as fixtures -- the conformance
        # surface HOLDEM-PROTOCOL.md section 15 points porters at. Same values
        # as --print-pinned; only the framing differs.
        import json
        print(json.dumps(compute_all(), indent=1, sort_keys=True))
        return 0
